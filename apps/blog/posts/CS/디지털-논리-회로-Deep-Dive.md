---
title: 디지털 논리 회로 Deep Dive
date: 2026-04-28
---

디지털 논리회로 과목을 3강 정도 듣다 포기했었는데, '컴퓨터 밑바닥의 비밀'이라는 책을 통해 다시 흥미가 생겼다. 책을 읽으면서 든 생각을 시작으로, 디지털 논리회로의 기본적인 내용을 정리해봤다.

## 모든 것은 CPU에서 시작된다

우리가 쓴 코드들은 결국 컴퓨터가 해석할 수 있는 0과 1로 이루어진 기계어가 담긴 실행 파일로 변환된다. CPU는 이 데이터와 명령어를 가지고 연산을 수행한다. '그렇다면 프로그램들을 어떻게 동시에 돌릴 수 있을까?'란 고민에서 운영체제, 프로세스 스케쥴링 같은 개념들이 나온다. 

정처기나 많은 CS 책들이 프로세스는 실행 중인 프로그램, 스레드는 프로세스 실행 흐름의 단위처럼 설명하지만 CS 개념들은 결국 CPU라는 한정된 자원을 더 효율적으로 사용하기 위한 것에서 출발한다는 생각이 들었다. 디지털 논리회로는 CPU가 이런 명령어와 데이터를 처리하는 과정을 조금 더 이해할 수 있게 해준다.

우리는 컴퓨터 안에서 어떤 일이 일어나는지 몰라도 사용할 수 있지만

```ts
// 돈만 줘 나는 그린애플을 줄게
function getGreenApple(money: number): GreenApple {
  // 과정은 몰라도 돼
  return greenApple;
}
```
