...
RSC는 기존 SSR 방식의 문제를 해결하기 위해 등장했다. 당시 SSR의 가장 큰 문제는 컴포넌트가 서버와 클라이언트에서 **이중 호출**된다는 것이었다.
...
1. 1. **애초에 서버 컴포넌트는 서버에서만 실행된다.** 이 과정에서 이모션이 런타임에 생성한 결과가 브라우저로 전달되지 못한다. 하지만 use client를 붙이면 클라이언트 컴포넌트가 되어 브라우저 환경에서 다시 실행된다.
